...
save_dir = os.path.join(os.path.dirname(os.path.dirname(current_dir)), 'output', 'dual', 'MobileNetV3')
...
class DualMobileNetV3(nn.Module):
    def __init__(self):
        super().__init__()
        self.orig_branch = get_custom_mobilenet_v3()
        self.diff_branch = get_custom_mobilenet_v3()
...
model = DualMobileNetV3().to(device)
