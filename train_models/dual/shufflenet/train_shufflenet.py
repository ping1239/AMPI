...
save_dir = os.path.join(os.path.dirname(os.path.dirname(current_dir)), 'output', 'dual', 'ShuffleNet')
...
class DualShuffleNet(nn.Module):
    def __init__(self):
        super().__init__()
        self.orig_branch = get_custom_shufflenet()
        self.diff_branch = get_custom_shufflenet()
...
model = DualShuffleNet().to(device)
